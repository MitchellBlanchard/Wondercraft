...
void Controller::inputs() {
	...
	*/
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) {
		...
		std::cout << (model->player->position.y * view->TILE_SIZE) << std::endl;
		sf::Vector2f playerVec = sf::Vector2f(((-view->getStartingPos().x)*view->TILE_SIZE) + (model->player->position.x * view->TILE_SIZE), fmod(((view->getStartingPos().y*view->TILE_SIZE) + (model->player->position.y * view->TILE_SIZE)) , view->windowSize.y));
		model->playerProjectiles.push_back(new Projectile(model->player->position,playerVec, mouseVec));
